...
occ_raw <- read.csv("50yrss74-24.csv")
...
if (!is.null(pres.locs) && nrow(pres.locs) >= 2) {
  ...
  writeRaster(bias.ras, "set1bias.asc", format = "ascii", overwrite = TRUE)
  message("Bias file created: set1bias.asc")
} else {
...
# ==== 6. Save results ====
write.csv(enmeval_results@results, "set1enmeval_results_block.csv")
message("Model tuning complete. Results saved to enmeval_results_block.csv")
...
if (!is.null(pres.locs) && nrow(pres.locs) >= 2) {
  ...
  extent(bias.ras) <- extent(env)
  bias.ras <- resample(bias.ras, env)
  writeRaster(bias.ras, "set2bias.asc", format = "ascii", overwrite = TRUE)
  message("Bias file created: set2bias.asc")
} else {
  warning("Fewer than 2 presence points; using uniform bias raster.")
  bias.ras <- env[[1]]
  values(bias.ras) <- 1
...
# ==== 6. Save results ====
write.csv(enmeval_results@results, "set2enmeval_results_block.csv")
message("Model tuning complete. Results saved to enmeval_results_block.csv")
...
if (!is.null(pres.locs) && nrow(pres.locs) >= 2) {
  ...
  extent(bias.ras) <- extent(env)
  bias.ras <- resample(bias.ras, env)
  writeRaster(bias.ras, "set3bias.asc", format = "ascii", overwrite = TRUE)
  message("Bias file created: set3bias.asc")
} else {
  warning("Fewer than 2 presence points; using uniform bias raster.")
  bias.ras <- env[[1]]
  values(bias.ras) <- 1
...
# ==== 6. Save results ====
write.csv(enmeval_results@results, "set3enmeval_results_block.csv")
message("Model tuning complete. Results saved to set3enmeval_results_block.csv")
...
if (!is.null(pres.locs) && nrow(pres.locs) >= 2) {
  ...
  extent(bias.ras) <- extent(env)
  bias.ras <- resample(bias.ras, env)
  writeRaster(bias.ras, "set4bias.asc", format = "ascii", overwrite = TRUE)
  message("Bias file created: set4bias.asc")
} else {
  warning("Fewer than 2 presence points; using uniform bias raster.")
  bias.ras <- env[[1]]
  values(bias.ras) <- 1
...
# ==== 6. Save results ====
write.csv(enmeval_results@results, "set4enmeval_results_block.csv")
message("Model tuning complete. Results saved to enmeval_results_block.csv")
